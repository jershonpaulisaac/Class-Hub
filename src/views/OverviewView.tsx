...
export function OverviewView({ onNavigate }: { onNavigate: (id: TabId) => void }) {
  ...
  const todayClasses = (timetable ?? []).filter((c) => c.day_of_week === todayDow());
  const now = nowHHMM();
            ...
            <Card hover className="group cursor-pointer overflow-hidden p-0">
              <button onClick={() => onNavigate('events')} className="block w-full text-left">
                <div className="relative">
                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/20 to-sky-500/10" />
        ...
        <div className="space-y-6">
          <Card className="p-0">
            ...
            <div className="divide-y divide-slate-800">
              {(notices ?? []).length === 0 ? (
                <p className="px-5 py-8 text-center text-xs text-slate-500">No notices.</p>
              ) : (
                (notices ?? []).map((n) => (
          ...
          <Card className="p-0">
            ...
            <div className="divide-y divide-slate-800">
              {upcomingAssignments.length === 0 ? (
                <p className="px-5 py-8 text-center text-xs text-slate-500">All caught up.</p>
              ) : (
